...
document.addEventListener("DOMContentLoaded", function () {
  ...
  btn.innerHTML =
    '<svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"></polyline></svg> Retour';
...
document.addEventListener("DOMContentLoaded", function () {
  const metadataDl = document.querySelector(
    ".resource.show dl, .block-itemWithMetadata .resource dl",
  );
  ...
  const iiifLink = document.querySelector(
    'link[rel="alternate"][type="application/ld+json"][href*="manifest"]',
  );
  ...
  if (!manifestUrl && window.location.pathname.includes("/item/")) {
    ...
    if (matches && matches[1]) {
      manifestUrl =
        window.location.origin + "/iiif/2/" + matches[1] + "/manifest";
    }
  }
  if (!manifestUrl && typeof window.miradors !== "undefined") {
    ...
    if (miradorKeys.length > 0) {
      ...
      if (configM && configM.windows && configM.windows.length > 0) {
        manifestUrl =
          configM.windows[0].manifestId || configM.windows[0].loadedManifest;
      }
    }
  }
  ...
  const checkSvg =
    '<svg viewBox="0 0 24 24" width="20" height="20" stroke="var(--accent)" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
  ...
  btnReport.href =
    emailHref +
    "?subject=Signalement sur la page " +
